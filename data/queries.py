...
def get_shows_with_genres():
    return data_manager.execute_select('''
            SELECT shows.id, shows.title, ROUND (shows.rating) AS rating, COUNT ( DISTINCT show_characters.actor_id) AS number_of_actors
            FROM shows
            INNER JOIN show_genres ON shows.id = show_genres.show_id
            LEFT JOIN show_characters ON shows.id = show_characters.show_id
            GROUP BY shows.id
            ORDER BY count( DISTINCT show_genres.genre_id)
            LIMIT 10''')
